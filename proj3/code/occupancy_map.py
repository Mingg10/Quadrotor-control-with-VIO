...
class OccupancyMap:
    ...
    def find_neighbor_index(self, voxel_index):
        """
        Find the neighboring voxel indexes.
        All occupied or outside the map neighbors have been removed.
        """
        x, y, z = voxel_index
        self.neighbors = []     # list to store all theoretical neighbors indexes
        for dx, dy, dz in self.neighbor_offsets:
            neighbor = np.array((x + dx, y + dy, z + dz))
            if not self.is_occupied_index(neighbor):
...
if __name__ == "__main__":
    ...
    # Draw the occupancy map (maybe slow for many voxels; will look weird if plotted on top of a world.draw)
    oc.draw(ax)
